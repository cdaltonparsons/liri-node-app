...
inquirer
  .prompt([
    ...
    {
      type: "input",
      message: "And what song, band, or movie would you like me to look up? (Please no special characters, i.e. periods (.) or apostrophes ('))",
      name: "thing"
    }
  ])
...
function spotifyThis(answer) {
  if (answer) {
    ...
    console.log(
      "\nYou forgot to input a song, so here is information on my favorite song, 'The Sign is by: " + response.tracks.items[5].artists[0].name
    );
    console.log("\n");
    ...
    console.log("\n");
    console.log(
      "It appears on the album: " + response.tracks.items[5].album.name + "\n"
    );
  }
})
}
